...
function LinkedPhonesSection() {
  const [phones,  setPhones]  = useState<LinkedPhone[]>([])
  const [loading, setLoading] = useState(true)
  const [adding,  setAdding]  = useState(false)
  ...
  useEffect(() => {
    fetch('/api/settings/phones')
      .then((r) => r.json())
      .then((j) => { if (j.phones) setPhones(j.phones) })
      .finally(() => setLoading(false))
  }, [])
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    setErr(null)
    setSaving(true)
    const res  = await fetch('/api/settings/phones', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const json = await res.json()
    setSaving(false)
    if (!res.ok) { setErr(json.error ?? 'Failed to add number'); return }
    setPhones((prev) => [...prev, json.phone])
    setNewPhone('')
    setNewLabel('')
    setAdding(false)
  }
  ...
  async function handleDelete(id: string) {
    setDeletingId(id)
    await fetch(`/api/settings/phones/${id}`, { method: 'DELETE' })
    setPhones((prev) => prev.filter((p) => p.id !== id))
    setDeletingId(null)
  }

  return (
    <Section title="Login phone numbers">
      <p className="text-xs text-zinc-500 -mt-2">
        Register phone numbers for SMS login. Enter any registered number at the login
        screen to receive a one-time code via text message.
      </p>
        ...
        <div className="space-y-2">
          {phones.map((item) => (
            <div key={item.id} className="flex items-center gap-3 rounded-lg border border-zinc-800 bg-zinc-900 px-3 py-2.5">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-zinc-200 font-mono truncate">{item.phone}</p>
                {item.label && <p className="text-[10px] text-zinc-500">{item.label}</p>}
              </div>
              <button
                onClick={() => handleDelete(item.id)}
            ...
            <form onSubmit={handleAdd} className="rounded-lg border border-zinc-700 bg-zinc-800/50 p-3 space-y-3">
              <div className="space-y-2">
                <input
                  ...
                  className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition font-mono"
                />
                <input
                  type="text"
                  placeholder="Label (optional — e.g. Mobile, Work)"
                  value={newLabel}
                  onChange={(e) => setNewLabel(e.target.value)}
                  className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-white placeholder-zinc-600 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition"
                />
                <p className="text-[11px] text-zinc-600">Include country code, e.g. +31612345678</p>
              </div>
              {err && <p className="text-xs text-red-400">{err}</p>}
              <div className="flex gap-2">
                <button
                ...
                <button
                  type="button"
                  onClick={() => { setAdding(false); setNewPhone(''); setNewLabel(''); setErr(null) }}
                  className="rounded-lg border border-zinc-700 px-3 py-1.5 text-xs text-zinc-400 hover:text-white hover:bg-zinc-800 transition"
                >
                  Cancel
                </button>
